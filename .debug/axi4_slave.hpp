        ...
        void write_beat(axi4_ref <A_WIDTH,D_WIDTH,ID_WIDTH> &pin) {
            if (pin.wvalid && pin.wready) {
                ...
                uint64_t addr_base = w_current_addr;
                        printf("old w_current_addr:%lx ",w_current_addr);
                w_current_addr += w_each_len - (addr_base % w_each_len);//+ 8 - (138%8=0)
                ...
                for (std::pair<int,int> sub_range : range) {
                    printf("strb: %d",pin.wstrb);
                    int &addr = sub_range.first;
                    int &len  = sub_range.second;
                    printf("%lx, %d\n",addr_base+addr, len);
                    memcpy(w_buffer,&(pin.wdata),sizeof(pin.wdata));
                    printf("w_current_addr:%lx, addr_base:%lx, addr:%lx, D_bytes:%d, len:%d, in_data_pos:%d, rem_data_pos:%d\n",w_current_addr,addr_base,addr,D_bytes,len,in_data_pos,rem_data_pos);
                    printf("**********\n");
                    for(int i = 0; i < 8; i++){
                        unsigned char a = (unsigned char)(w_buffer[i]);
                        printf("0x%lx %02x\n", addr_base+addr + i, a);
                    }
