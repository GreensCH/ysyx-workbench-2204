...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 0000011", lbu    , I, R(dest) = 0xF  & BITS(Mr(src1 + src2, 1), 7, 0););
  INSTPAT("??????? ????? ????? 101 ????? 0000011", lhu    , I, R(dest) = 0xFF & BITS(Mr(src1 + src2, 2), 15, 0););
  INSTPAT("??????? ????? ????? 000 ????? 0100011", sb     , S, Mw(src1 + dest, 1, BITS(src2, 7, 0)));
  INSTPAT("??????? ????? ????? 000 ????? 0100011", sh     , S, Mw(src1 + dest, 2, BITS(src2, 15, 0)));
  INSTPAT("??????? ????? ????? 000 ????? 0100011", sw     , S, Mw(src1 + dest, 4, BITS(src2, 31, 0)));
  //I-type | CALI
  ...
  INSTPAT("??????? ????? ????? 111 ????? 0010011", andi   , I, R(dest) = src1 & SEXT(src2, 12));
  INSTPAT("000000 ?????? ????? 111 ????? 0010011", slli   , I, R(dest) = src1 << src2);
  INSTPAT("000000 ?????? ????? 101 ????? 0010011", srli   , I, R(dest) = (word_t)((word_t)src1  >> src2));
  INSTPAT("010000 ?????? ????? 101 ????? 0010011", srai   , I, R(dest) = (word_t)((sword_t)src1 >> src2));
  //R-type | CALR
  ...
  INSTPAT("??????? ????? ????? 110 ????? 0000011", lwu    , I, R(dest) = 0xFFFF & BITS(Mr(src1 + src2, 4), 31, 0));
  INSTPAT("??????? ????? ????? 000 ????? 0011011", addiw  , I, SEXT(BITS(src1 + src2, 31, 0), 32););
  // INSTPAT("0000000 ????? ????? 001 ????? 0011011", slliw  , I, R(dest) = src1 << src2;);
  // INSTPAT("0000000 ????? ????? 101 ????? 0011011", srliw  , I, R(dest) = (src1 & 0xFFFF) >> src2;);
  // INSTPAT("0100000 ????? ????? 101 ????? 0011011", sraiw  , I, R(dest) = src1 >> src2;);
  INSTPAT("0000000 ????? ????? 000 ????? 0111011", addw   , R, SEXT(BITS(src1 + src2, 31, 0), 32););
  INSTPAT("0100000 ????? ????? 000 ????? 0111011", subw   , R, SEXT(BITS(src1 - src2, 31, 0), 32););
