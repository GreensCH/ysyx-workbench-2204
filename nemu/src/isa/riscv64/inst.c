...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  //J-type
  INSTPAT("??????? ????? ????? ??? ????? 1101111", jal    , J, R(dest) = s->pc + 4lu; s->dnpc = s->pc + (sword_t)SEXT(src1, 20/*21*/); );
  //B-type
  INSTPAT("??????? ????? ????? 000 ????? 1100011", beq    , B, s->dnpc = (src1 == src2) ? s->pc + SEXT(dest, 12/*13*/) : s->dnpc);
  INSTPAT("??????? ????? ????? 001 ????? 1100011", bne    , B, s->dnpc = (src1 != src2) ? s->pc + SEXT(dest, 12/*13*/) : s->dnpc);
  INSTPAT("??????? ????? ????? 001 ????? 1100011", blt    , B, s->dnpc = ((sword_t)src1 < (sword_t)src2) ? s->pc + SEXT(dest, 12/*13*/) : s->dnpc);
  //I-type
