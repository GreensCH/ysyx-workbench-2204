...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1==src2) s->dnpc = s->pc + SEXT(dest, 13););
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1!=src2) s->dnpc = s->pc + SEXT(dest, 13););
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((long long)src1 < (long long)src2) s->dnpc = s->pc + SEXT(dest, 13););
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((long long)src1 >= (long long)src2) s->dnpc = s->pc + SEXT(dest, 13););
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if(src1 < src2) s->dnpc = s->pc + SEXT(dest, 13););
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if(src1 >= src2) s->dnpc = s->pc +  SEXT(dest, 13););
  //I-type | Load-Store
