...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  word_t zimm = src1;
  INSTPAT("??????? ????? ????? 001 ????? 1110011", csrrw , R, printf("csrrw %ld\n",src1);word_t t = SR(csr); SR(csr) = R(src1); R(dest) = t;); 
  INSTPAT("??????? ????? ????? 010 ????? 1110011", csrrs , R, printf("csrrs\n");word_t t = SR(csr); SR(csr) = t | R(src1) ; R(dest) = t;); 
  INSTPAT("??????? ????? ????? 011 ????? 1110011", csrrc , R, printf("csrrc\n");word_t t = SR(csr); SR(csr) = t & ~R(src1); R(dest) = t;); 
  INSTPAT("??????? ????? ????? 101 ????? 1110011", csrrwi, R, printf("csrrwi\n"); R(dest) = SR(csr); SR(csr) = zimm); 
  INSTPAT("??????? ????? ????? 110 ????? 1110011", csrrsi, R, printf("csrrsi\n");word_t t = SR(csr); SR(csr) = t | zimm ; R(dest) = t;);  
  INSTPAT("??????? ????? ????? 111 ????? 1110011", csrrci, R, printf("csrrci\n");word_t t = SR(csr); SR(csr) = t & ~zimm; R(dest) = t;); 
