...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  // //B-type
  // INSTPAT("??????? ????? ????? 000 ????? 1100011", beq    , B, s->pc = (src1 == src2) ? s->pc + SEXT(dest, 12) : s->npc);
  // INSTPAT("??????? ????? ????? 001 ????? 1100011", bne    , B, s->pc = (src1 != src2) ? s->pc + SEXT(dest, 12) : s->npc);
  // INSTPAT("??????? ????? ????? 001 ????? 1100011", blt    , B, s->pc = ((sword_t)src1 < (sword_t)src2) ? s->pc + SEXT(dest, 12));
  //I-type
