...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 1100011", blt    , B, s->dnpc = ((sword_t)src1 < (sword_t)src2) ? s->pc + SEXT(dest, 12/*13*/) : s->dnpc);
  INSTPAT("??????? ????? ????? 101 ????? 1100011", bge    , B, s->dnpc = ((sword_t)src1 >= (sword_t)src2) ? s->pc + SEXT(dest, 12/*13*/) : s->dnpc);
  INSTPAT("??????? ????? ????? 110 ????? 1100011", bltu   , B, s->dnpc = ((word_t)src1 < (word_t)src2) ? s->pc + SEXT(dest, 12/*13*/) : s->dnpc);
  INSTPAT("??????? ????? ????? 111 ????? 1100011", bgeu   , B, s->dnpc = ((sword_t)src1 < (sword_t)src2) ? s->pc + SEXT(dest, 12/*13*/) : s->dnpc);
  //I-type
