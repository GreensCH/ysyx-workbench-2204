...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000000 00000 00000 000 00000 1110011", ecall , N, mepc = s->pc; mcause = 0x8000000000000000 | 11; mstatus = 0 | (0b11 << 11) | (BITS(mstatus, 3, 3) << 7) | (0b0 << 3); s->dnpc = mtvec);
  ...
  word_t zimm = src1;
  INSTPAT("??????? ????? ????? 001 ????? 1110011", csrrw , N, word_t t = SR(csr); SR(csr) = R(src1); R(dest) = t;); 
  INSTPAT("??????? ????? ????? 010 ????? 1110011", csrrs , N, word_t t = SR(csr); SR(csr) = t | R(src1) ; R(dest) = t;); 
  INSTPAT("??????? ????? ????? 011 ????? 1110011", csrrc , N, word_t t = SR(csr); SR(csr) = t & ~R(src1); R(dest) = t;); 
  INSTPAT("??????? ????? ????? 101 ????? 1110011", csrrwi, N,  R(dest) = SR(csr); SR(csr) = zimm); 
  INSTPAT("??????? ????? ????? 110 ????? 1110011", csrrsi, N, word_t t = SR(csr); SR(csr) = t | zimm ; R(dest) = t;);  
  INSTPAT("??????? ????? ????? 111 ????? 1110011", csrrci, N, word_t t = SR(csr); SR(csr) = t & ~zimm; R(dest) = t;); 
