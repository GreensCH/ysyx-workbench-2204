...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 0011011", addiw  , I, R(dest) = SEXT(BITS(src1 + SEXT(src2, 12),31,0), 32));
  INSTPAT("0000000 ????? ????? 001 ????? 0011011", slliw  , I, R(dest) = SEXT((word_t)src1 << src2, 32));
  INSTPAT("0000000 ????? ????? 101 ????? 0011011", srliw  , I, R(dest) = SEXT((word_t)src1 >> BITS(src2 ,5 ,0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 0011011", sraiw  , I, R(dest) = SEXT((sword_t)src1 >> BITS(src2 ,5 ,0), 32));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 0111011", srlw   , R, R(dest) = SEXT((word_t)src1 >> BITS(src2, 4, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 0111011", sraw   , R, R(dest) = SEXT((sword_t)src1 >> BITS(src2, 4, 0), 32));
