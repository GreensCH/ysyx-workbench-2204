...
//    val pc = Output(UInt(CacheCfg.paddr_bits.W))
//  }
//}
...
//   Main Data Reference
//   */
//  protected val prev_index  = prev.bits.pc(index_border_up, index_border_down)//(8, 4)
//  protected val prev_tag    = prev.bits.pc(tag_border_up, tag_border_down)
//  protected val stage_index = lkup_stage_out.bits.pc(index_border_up, index_border_down)
//  protected val stage_tag   = lkup_stage_out.bits.pc(tag_border_up, tag_border_down)
//  /*
...
//  protected val miss = miss1 | miss2 | miss3
//  dontTouch(miss1)
//  dontTouch(miss2)
//  dontTouch(miss3)
...
//   */
//  a_addr := Cat(lkup_stage_out.bits.pc(38, 4), 0.U(4.W))// axi read addr
//  lkup_stage_in.bits.pc := prev.bits.pc
...
//  bus_out.data.if2id.pc := lkup_stage_out.bits.pc
//  bus_out.data.if2id.inst := MuxLookup(key = lkup_stage_out.bits.pc(3, 2), default = 0.U(32.W), mapping = Array(
...
//  cache_out.data.if2id.pc := lkup_stage_out.bits.pc
//  cache_out.data.if2id.inst := MuxLookup(key = lkup_stage_out.bits.pc(3, 2), default = 0.U(32.W), mapping = Array(
...
//    pc_old := prev.bits.pc
//    when(curr_state === sLOOKUP & (pc_old =/= prev.bits.pc)){
//      when(tag0_hit ){
