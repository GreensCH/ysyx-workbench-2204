...
class EXU extends Module{
  ...
  val adder_in2 = Mux(operator.sub, (alu_src2 ^ (-1.U(64.W))) + 1.U(64.W), alu_src2)
  val adder_out = adder_in1 + adder_in2
  /* result generator */
  val res = MuxCase(adder_out,
    Array(
      (operator.auipc ) -> (src1 + src2),
      (operator.lui   ) -> src1,
      (operator.jal | operator.jalr) -> (4.U + src1),
      (operator.sb    ) -> src2,
      (operator.sh    ) -> src2,
      (operator.sw    ) -> src2,
      (operator.sd    ) -> src2,
  ...
  val result_out = MuxCase(res,
    Array(
      byte  -> Util.sext(res, pos = 8),
      hword -> Util.sext(res, pos = 16),
      word  -> Util.sext(res, pos = 32),
      dword -> Util.sext(res, pos = 64),
    )
  )
  /* ex2mem interface */
