...
//  printf("EXU\t\n")
  ...
  val res = MuxCase(0.U,
    Array(
      (operator.auipc ) -> (src1 + src2),//src2 = p.asUInt()c
      (operator.lui   ) -> src1.asUInt(),
      (operator.jal | operator.jalr) -> (4.U + src2),//src1 = 0, src = p.asUInt()c
      (operator.sb    ) -> src2.asUInt(),
      ...
      (operator.mul   ) -> 0.U.asUInt(),
      (operator.mulh  ) -> 0.U.asUInt(),
      (operator.mulhu ) -> 0.U.asUInt(),
      (operator.mulhsu) -> 0.U.asUInt(),
      (operator.div   ) -> 0.U.asUInt(),
      (operator.divu  ) -> 0.U.asUInt(),
      (operator.rem   ) -> 0.U.asUInt(),
      (operator.remu  ) -> 0.U.asUInt(),
    )
  )
