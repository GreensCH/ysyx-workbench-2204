   ...
   */
    ...
    */
    ...
    when(curr_state === sWRITE_1){
      AXI4BundleW.set(inf = maxi.w, id= 0.U, data = wdata(63, 0), strb = "hff".U, last = !overborder)
    }
    .elsewhen(curr_state === sWRITE_2){
      AXI4BundleW.set(inf = maxi.w, id= 0.U, data = wdata(127, 64), strb = "hff".U, last = true.B)
    }
    AXI4BundleB.default(maxi.b)
    dontTouch(maxi.b)
    /*
