...
class ICache extends Module{
  ...
  .elsewhen(curr_state === sRWRITE){
    cache_valid := temp_valid_reg// this may be same as prev.valid, but could cause unpredicted problem
    cache_ready := false.B
  }
 // AXI Control Signal
...
val inst_out = MuxLookup(key = pc_index, default = 0.U(32.W), mapping = Array(
  ...
  when(last){
    temp_data_reg.if2id.inst := inst_out
  }.otherwise{
  ...
  when(next_state === sRWRITE){
    when(lru_list(index) === 0.U){// last is 0
      ...
      tag_array_io_1  <> SRAMIO.write(index, data_array_in)
    }
    .otherwise{
      lru_list(index) := 0.U//now the last is 0
      data_array_io_0 <> SRAMIO.write(index, data_array_in)
      tag_array_io_0  <> SRAMIO.write(index, data_array_in)
    }
  }.otherwise{
  ...
  val tag0_hit = tag_array_io_0.rdata === prev.bits.pc2if.pc(tag_border_up, tag_border_down)
  val tag1_hit = tag_array_io_1.rdata === prev.bits.pc2if.pc(tag_border_up, tag_border_down)
// Data Output
