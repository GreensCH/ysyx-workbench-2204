...
void mtrace_rd_log(word_t data, word_t addr){
  bool flag = true;
  if (likely(in_pmem(addr))) printf("PMEM-RD:PC(0x%016lx) data(0x%016lx) addr(0x%016lx)", isa_reg_str2val("PC", &flag), data, addr); 
...
void mtrace_we_log(word_t data, word_t addr){
  bool flag = true;
  if (likely(in_pmem(addr))) printf("PMEM-WE:PC(0x%016lx) data(0x%016lx) addr(0x%016lx)", isa_reg_str2val("PC", &flag), data, addr); 
