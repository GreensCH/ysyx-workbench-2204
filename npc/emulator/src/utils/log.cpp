...
void mtrace_rd_log(word_t data, word_t addr){
  bool flag = true;
  if (likely(in_pmem(addr))) Log("PMEM-RD:PC(0x%016lx) data(0x%016lx) addr(0x%016lx)", 1, data, addr); 
...
void mtrace_we_log(word_t data, word_t addr){
  bool flag = true;
  if (likely(in_pmem(addr))) Log("PMEM-WE:PC(0x%016lx) data(0x%016lx) addr(0x%016lx)", 1, data, addr); 
