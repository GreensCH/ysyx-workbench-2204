...
void mtrace_rd_log(word_t data, word_t addr){
  ...
  if (likely(in_pmem(addr))) Log("PMEM-RD:PC(0x%016lx) 0x%016lx @0x%016lx", cpu.pc, data, addr); 
  IFDEF(CONFIG_DEVICE, Log("MMIO-RD:PC(0x%016lx) 0x%016lx @0x%016lx", cpu.pc, data, addr)); 
}
void mtrace_we_log(word_t data, word_t addr){
  bool flag = true;
  // if(cpu_npc == addr) return;
  if (likely(in_pmem(addr))) Log("PMEM-WE:PC(0x%016lx) 0x%016lx @0x%016lx", cpu.pc, data, addr); 
  IFDEF(CONFIG_DEVICE, Log("MMIO-WE:PC(0x%016lx) 0x%016lx @0x%016lx", cpu.pc, data, addr)); 
}
#endif
